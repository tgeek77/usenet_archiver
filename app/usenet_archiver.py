#!/usr/bin/env python3
"""Usenet Archiver — fetch NNTP articles into mbox (sinntp-parity features).

Keep this file runnable as ``python3 app/usenet_archiver.py`` (and via symlink).
"""
...
# Allow sibling imports when run as a script or via symlink on $PATH.
_APP_DIR = os.path.dirname(os.path.abspath(__file__))
if _APP_DIR not in sys.path:
    sys.path.insert(0, _APP_DIR)

from creds import resolve_credentials  # noqa: E402
from fetch import fetch_articles_parallel  # noqa: E402
from mboxout import MboxWriter  # noqa: E402
from nntp import NNTPClient, NNTPError  # noqa: E402
from overview import find_articles_in_date_range  # noqa: E402
from plugins import apply_plugins, parse_plugin_spec  # noqa: E402
...
def pull_group(
    ...
    # Skip if already recorded as completed (same job name).
    try:
        if os.path.exists(completed_log):
            with open(completed_log, encoding="utf-8") as fh:
                done = {line.strip() for line in fh if line.strip()}
            if mbox_filename in done:
                logger.info("Skipping %s (already in %s)", mbox_filename, completed_log)
                return
    except OSError as e:
